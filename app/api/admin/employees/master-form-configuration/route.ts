...
import {
  isEmployeeMasterFormConfigurationMutationError,
  resetEmployeeMasterFormConfiguration,
  saveEmployeeMasterFormConfiguration,
} from "@/src/services/employees/employeeMasterFormConfigurationMutation.service";
...
function getChangedFields(args: {
  before: EmployeeMasterFormResolvedConfiguration;
  after: EmployeeMasterFormResolvedConfiguration;
}) {
  return EMPLOYEE_MASTER_FORM_CONFIGURATION_FIELD_DEFINITIONS.filter(
    ({ fieldKey }) =>
...
export async function PUT(req: Request) {
  try {
    ...
    const before = await resolveEmployeeMasterFormConfiguration({ companyId });
    const body = await req.json().catch(() => null);

    const after = await saveEmployeeMasterFormConfiguration({
      companyId,
      input: {
    ...
    await writeAudit({
      ...
      targetId: EMPLOYEE_MASTER_FORM_CONFIGURATION_SCREEN_KEY,
      details: {
        op: "EMPLOYEE_MASTER_FORM_CONFIGURATION_SAVE",
        companyId,
    ...
    if (isEmployeeMasterFormConfigurationMutationError(err)) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const after = await resetEmployeeMasterFormConfiguration({
      companyId,
      screenKey:
    ...
    await writeAudit({
      ...
      targetId: EMPLOYEE_MASTER_FORM_CONFIGURATION_SCREEN_KEY,
      details: {
        op: "EMPLOYEE_MASTER_FORM_CONFIGURATION_RESET",
        companyId,
        screenKey: EMPLOYEE_MASTER_FORM_CONFIGURATION_SCREEN_KEY,
        changedFields: getChangedFields({ before, after }),
        before,
