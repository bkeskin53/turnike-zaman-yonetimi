...
import {
  isEmployeeMasterHistoryListConfigurationMutationError,
  resetEmployeeMasterHistoryListConfiguration,
  saveEmployeeMasterHistoryListConfiguration,
} from "@/src/services/employees/employeeMasterHistoryListConfigurationMutation.service";
...
function getChangedFields(args: {
  before: EmployeeMasterHistoryListResolvedConfiguration;
  after: EmployeeMasterHistoryListResolvedConfiguration;
}) {
  return EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_FIELD_DEFINITIONS.filter(
    ({ fieldKey }) =>
...
export async function PUT(req: Request) {
  try {
    ...
    const before = await resolveEmployeeMasterHistoryListConfiguration({
      companyId,
    });
    const body = await req.json().catch(() => null);

    const after = await saveEmployeeMasterHistoryListConfiguration({
      companyId,
      input: {
    ...
    await writeAudit({
      ...
      targetId: EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_SCREEN_KEY,
      details: {
        op: "EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_SAVE",
        companyId,
    ...
    if (isEmployeeMasterHistoryListConfigurationMutationError(err)) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const after = await resetEmployeeMasterHistoryListConfiguration({
      companyId,
      screenKey:
    ...
    await writeAudit({
      ...
      targetId: EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_SCREEN_KEY,
      details: {
        op: "EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_RESET",
        companyId,
        screenKey: EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_SCREEN_KEY,
        changedFields: getChangedFields({ before, after }),
        before,
