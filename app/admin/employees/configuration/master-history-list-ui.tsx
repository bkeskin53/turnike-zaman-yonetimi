...
function humanizeError(code: string | null, fallback: string): string {
  switch (code) {
    case "INVALID_EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_SCREEN_KEY":
      return "Screen key gecersiz.";
    case "INVALID_EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_FIELD_KEY":
      return "Ayar alanlarindan biri gecersiz.";
    case "INVALID_EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_FIELD_SET":
      return "Gecerli ve tam field set gonderilmelidir.";
    case "INVALID_EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_VISIBILITY":
...
export default function EmployeeMasterHistoryListConfigurationClient(props: {
  initialConfiguration: EmployeeMasterHistoryListResolvedConfiguration;
}) {
  ...
  const isDirty = useMemo(
    () =>
      EMPLOYEE_MASTER_HISTORY_LIST_CONFIGURATION_FIELD_DEFINITIONS.some(
        ({ fieldKey }) =>
  ...
  async function saveConfiguration() {
    ...
    try {
      const response = await fetch(
        "/api/admin/employees/master-history-list-configuration",
        {
          method: "PUT",
          headers: {
      ...
      setNotice({
        kind: "success",
        text: "Master tarihce liste ozeti ayarlari kaydedildi.",
      });
    } catch {
      setNotice({
        kind: "error",
  ...
  async function resetConfiguration() {
    ...
    try {
      ...
      setNotice({
        kind: "success",
        text: "Varsayilan master tarihce liste ozeti ayarlari geri yuklendi.",
      });
    } catch {
      setNotice({
        kind: "error",
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 pb-3">
          <div className="min-w-0">
            <div className="text-base font-semibold tracking-tight text-slate-950">
              Master Tarihce Liste Ozeti
            </div>
            <p className="mt-1 text-sm text-slate-500">
              Master tarihce dialogundaki sol liste ozeti.
            </p>
          </div>
