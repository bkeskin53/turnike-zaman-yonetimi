...
function humanizeError(code: string | null, fallback: string): string {
  switch (code) {
    case "INVALID_EMPLOYEE_CREATE_FORM_CONFIGURATION_SCREEN_KEY":
      return "Screen key gecersiz.";
    case "INVALID_EMPLOYEE_CREATE_FORM_CONFIGURATION_FIELD_KEY":
      return "Ayar alanlarindan biri gecersiz.";
    case "INVALID_EMPLOYEE_CREATE_FORM_CONFIGURATION_FIELD_SET":
      return "Gecerli ve tam field set gonderilmelidir.";
    case "INVALID_EMPLOYEE_CREATE_FORM_CONFIGURATION_VISIBILITY":
...
function buildFieldsPayload(
  fields: EmployeeCreateFormResolvedConfiguration["fields"],
) {
...
export default function EmployeeCreateFormConfigurationClient(props: {
  initialConfiguration: EmployeeCreateFormResolvedConfiguration;
}) {
  ...
  const isDirty = useMemo(
    () =>
      EMPLOYEE_CREATE_FORM_CONFIGURATION_FIELD_DEFINITIONS.some(
        ({ fieldKey }) =>
  ...
  async function saveConfiguration() {
    ...
    try {
      const response = await fetch(
        "/api/admin/employees/create-form-configuration",
        {
          method: "PUT",
          headers: {
      ...
      setNotice({
        kind: "success",
        text: "Gorunurluk ayarlari kaydedildi.",
      });
    } catch {
      setNotice({
        kind: "error",
  ...
  async function resetConfiguration() {
    ...
    try {
      ...
      setNotice({
        kind: "success",
        text: "Varsayilan gorunurluk ayarlari geri yüklendi.",
      });
    } catch {
      setNotice({
        kind: "error",
    ...
    <div className="grid min-w-0">
      <section className="h-full rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 pb-3">
          <div className="min-w-0">
            <div className="text-base font-semibold tracking-tight text-slate-950">
              Create Form Gorunurlugu
            </div>
            <p className="mt-1 text-sm text-slate-500">
              Yeni calisan ekleme formundaki opsiyonel alanlar.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-2">
          {EMPLOYEE_CREATE_FORM_CONFIGURATION_FIELD_DEFINITIONS.map(
            ({ fieldKey, label }) => {
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-3">
          <div className="text-xs text-slate-500">
            {isDirty
              ...
              : "Kayitli konfigürasyon aktif durumda."}
          </div>
