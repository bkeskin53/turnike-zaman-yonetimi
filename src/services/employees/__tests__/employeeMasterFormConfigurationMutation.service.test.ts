...
} from "../employeeMasterFormConfigurationMutation.service";
...
describe("employeeMasterFormConfigurationMutation", () => {
  it("rejects invalid screen keys", async () => {
    await expect(
      saveEmployeeMasterFormConfiguration({
        companyId: "company_1",
        input: {
          screenKey: "employees.create",
          fields: buildValidFields(),
        },
      }),
    ).rejects.toMatchObject({
      code: "INVALID_EMPLOYEE_MASTER_FORM_CONFIGURATION_SCREEN_KEY",
    });
  ...
  it("rejects invalid field keys", async () => {
    ...
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_FORM_CONFIGURATION_FIELD_KEY",
    });
  });
  ...
  it("rejects invalid visibility settings", async () => {
    ...
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_FORM_CONFIGURATION_VISIBILITY",
    });
  });
  ...
  it("rejects duplicate or incomplete field sets", async () => {
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_FORM_CONFIGURATION_FIELD_SET",
    });
  ...
  it("saves the exact visibility set and returns merged configuration", async () => {
    ...
    const resolved = await saveEmployeeMasterFormConfiguration({
      companyId: "company_1",
      input: {
        screenKey: EMPLOYEE_MASTER_FORM_CONFIGURATION_SCREEN_KEY,
        fields: buildValidFields({
  ...
  it("resets to persisted defaults by removing company overrides", async () => {
    ...
    const resolved = await resetEmployeeMasterFormConfiguration({
      companyId: "company_1",
      db: {
