...
} from "../employeeMasterHistoryFormConfigurationMutation.service";
...
describe("employeeMasterHistoryFormConfigurationMutation", () => {
  it("rejects invalid screen keys", async () => {
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_HISTORY_FORM_CONFIGURATION_SCREEN_KEY",
    });
  ...
  it("rejects invalid field keys", async () => {
    ...
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_HISTORY_FORM_CONFIGURATION_FIELD_KEY",
    });
  });
  ...
  it("rejects invalid visibility settings", async () => {
    ...
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_HISTORY_FORM_CONFIGURATION_VISIBILITY",
    });
  });
  ...
  it("rejects duplicate or incomplete field sets", async () => {
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_HISTORY_FORM_CONFIGURATION_FIELD_SET",
    });
  ...
  it("saves the exact visibility set and returns merged configuration", async () => {
    ...
    const resolved = await saveEmployeeMasterHistoryFormConfiguration({
      companyId: "company_1",
      input: {
        screenKey: EMPLOYEE_MASTER_HISTORY_FORM_CONFIGURATION_SCREEN_KEY,
        fields: buildValidFields({
  ...
  it("resets to persisted defaults by removing company overrides", async () => {
    ...
    const resolved = await resetEmployeeMasterHistoryFormConfiguration({
      companyId: "company_1",
      db: {
