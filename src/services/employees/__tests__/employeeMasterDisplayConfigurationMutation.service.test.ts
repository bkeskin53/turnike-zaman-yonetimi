...
} from "../employeeMasterDisplayConfigurationMutation.service";
...
describe("employeeMasterDisplayConfigurationMutation", () => {
  it("rejects invalid screen keys", async () => {
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_DISPLAY_CONFIGURATION_SCREEN_KEY",
    });
  ...
  it("rejects invalid field keys", async () => {
    ...
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_DISPLAY_CONFIGURATION_FIELD_KEY",
    });
  });
  ...
  it("rejects invalid visibility settings", async () => {
    ...
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_DISPLAY_CONFIGURATION_VISIBILITY",
    });
  });
  ...
  it("rejects duplicate or incomplete field sets", async () => {
    await expect(
      ...
      code: "INVALID_EMPLOYEE_MASTER_DISPLAY_CONFIGURATION_FIELD_SET",
    });
  ...
  it("saves the exact visibility set and returns merged configuration", async () => {
    ...
    const resolved = await saveEmployeeMasterDisplayConfiguration({
      companyId: "company_1",
      input: {
        screenKey: EMPLOYEE_MASTER_DISPLAY_CONFIGURATION_SCREEN_KEY,
        fields: buildValidFields({
    ...
    expect(upsert).toHaveBeenCalledWith({
      where: {
        companyId_screenKey: {
          companyId: "company_1",
          screenKey: ScreenConfigurationScreenKey.EMPLOYEES_MASTER_DISPLAY,
        },
      },
      create: {
        companyId: "company_1",
  ...
  it("resets to persisted defaults by removing company overrides", async () => {
    ...
    const resolved = await resetEmployeeMasterDisplayConfiguration({
      companyId: "company_1",
      db: {
    ...
    expect(resolved).toEqual(buildDefaultEmployeeMasterDisplayConfiguration());
  });
});
